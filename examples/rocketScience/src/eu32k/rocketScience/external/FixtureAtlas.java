package eu32k.rocketScience.external;
...
public class FixtureAtlas {
   ...
   private void importFromFile(FileHandle shapeFile) {
      DataInputStream is = null;
      try {
         is = new DataInputStream(new ByteArrayInputStream(shapeFile.readBytes()));
         while (true) {
      ...
      public Vector2[][] getPolygons(float width, float height) {
         for (int i = 0; i < normalizedPolygons.length; i++) {
            for (int ii = 0; ii < normalizedPolygons[i].length; ii++) {
               polygons[i][ii] = new Vector2(normalizedPolygons[i][ii]);
               polygons[i][ii].x -= 50f;
               polygons[i][ii].y -= 50f;
               polygons[i][ii].x *= width / 100f;
               polygons[i][ii].y *= height / 100f;
            }
         }
         return polygons;
      }
